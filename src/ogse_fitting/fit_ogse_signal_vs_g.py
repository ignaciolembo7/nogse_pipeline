...
def fit_ogse_signal_vs_g_long(
    ...
    *,
    ...
    for (dir_val, roi_val), d in dfa.groupby(["direction", "roi"], sort=False):
        ...
        if outdir_plots is not None:
            out_png = outdir_plots / f"{plot_prefix}{roi_val}.{model_name}.{g_type}.{ycol}.direction_{dir_val}.png"
    ...
    *,
    ...
    if not outs.fit_table.empty:
        for (roi_val, dir_val), grp in outs.fit_table.groupby(["roi", "direction"]):
            stem = f"{plot_prefix}{roi_val}.{model_name}.{g_type}.{ycol}.direction_{dir_val}"
            write_table_outputs(
