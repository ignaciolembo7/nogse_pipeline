...
def _apply_clean_aliases(df: pd.DataFrame) -> pd.DataFrame:
    out = _apply_renames(df)
    alias_map = {
        "signal_norm": "value_norm",
        "gthorsten": "g_thorsten",
        "gthorsten_mTm": "g_thorsten",
        "bvalue_gthorsten": "bvalue_thorsten",
    }
    rename = {src: dst for src, dst in alias_map.items() if src in out.columns and dst not in out.columns}
    if rename:
        out = out.rename(columns=rename)
    return out
...
def _ensure_clean_S0_and_norm(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ...
    if have_keys:
        s0 = (
            out.loc[pd.to_numeric(out["b_step"], errors="coerce") == 0]
            .groupby(["stat", "roi", "direction"], as_index=False)["value"]
            .mean()
            .rename(columns={"value": "S0"})
        )
        out = out.drop(columns=["S0"], errors="ignore").merge(s0, on=["stat", "roi", "direction"], how="left")
...
def _ensure_clean_bvalue_derivatives(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    if "bvalue" not in out.columns:
        raise KeyError("Missing 'bvalue' column (after renames).")
    ...
    if "g_thorsten" in out.columns and out["bvalue_thorsten"].isna().any():
        ...
        out["bvalue_thorsten"] = b_from_g(g, N=N, gamma=gamma, delta_ms=delta_ms, delta_app_ms=delta_app_ms, g_type="gthorsten")
...
def finalize_clean_signal_long(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = _ensure_clean_S0_and_norm(out)
