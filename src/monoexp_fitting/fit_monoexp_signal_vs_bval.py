...
def _require_strict_columns(df: pd.DataFrame) -> None:
    raise_on_unrecognized_column_names(df.columns, context="fit_monoexp_signal_vs_bval")
...
def _normalize_g_type(g_type: str) -> str:
    ...
    if raw not in VALID_G_TYPES:
        raise ValueError(
            f"fit_monoexp_signal_vs_bval: unrecognized g_type {raw!r}. "
...
def build_monoexp_fit_label(fit_row: dict[str, object]) -> str:
...
def plot_monoexp_signal_fit(
    *,
    ...
    fit_label = build_monoexp_fit_label(fit_row)
    render_xy_plot(
...
run_fit_monoexp_signal_vs_bval_from_parquet = run_fit_from_parquet
...
__all__ = [
    ...
    "fit_monoexp_signal_vs_bval_long",
    "fit_signal_vs_bval_long",
    "infer_exp_id",
    "monoexp",
    "run_fit_from_parquet",
    "run_fit_monoexp_signal_vs_bval_from_parquet",
]
