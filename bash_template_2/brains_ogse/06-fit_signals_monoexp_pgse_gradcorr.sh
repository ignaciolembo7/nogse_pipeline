...
export CORR_XLSX="${CORR_XLSX:-analysis/brains/ogse_experiments/fits/grad_correction_master/${CORR_ROI}.grad_correction.xlsx}"
export SIGNAL_FIT_EXTRA_ARGS="${SIGNAL_FIT_EXTRA_ARGS:-} --apply_grad_corr --corr_xlsx $CORR_XLSX --corr_roi $CORR_ROI"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec bash "$SCRIPT_DIR/../run_dataset.sh" brains fit_signal "$@"
