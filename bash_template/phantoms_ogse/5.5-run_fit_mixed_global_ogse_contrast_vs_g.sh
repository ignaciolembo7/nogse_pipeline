...
export ANALYSIS_ROOT="${ANALYSIS_ROOT:-$PROJECT_ROOT/analysis/phantoms-3/ogse_experiments}"
export TABLES_ROOT="${TABLES_ROOT:-$ANALYSIS_ROOT/contrast-data/tables}"
...
export ROIS="${ROIS:-ALL}"
export DIRECTIONS="${DIRECTIONS:-1 2 3}"
export GBASE="${GBASE:-g}"
export YCOL="${YCOL:-value_norm}"
export STAT="${STAT:-avg}"
export ONEG="${ONEG:-true}"
...
export CORR_XLSX="${CORR_XLSX:-$FITS_DIR/grad_correction/water.grad_correction.xlsx}"
...
if [[ ! -f "$ALPHA_TABLE" ]]; then
    ...
    echo "Run 3.3-run_make_alpha_macro_summary.sh first or set ALPHA_TABLE." >&2
    exit 1
fi
...
echo "============================================================"
echo "Phantoms-3 OGSE mixed-global contrast fit"
