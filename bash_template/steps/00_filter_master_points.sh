...
FILTERED_MASTER_PARQUET="${FILTERED_MASTER_PARQUET:-${MASTER_FIRST_POINTS_PARQUET:-$ANALYSIS_ROOT/master.first_points.long.parquet}}"

pipeline_require_file "$FILTER_MASTER_SCRIPT" "master filter script"
pipeline_require_file "$MASTER_PARQUET" "master table"
if [[ -z "${MASTER_FIRST_POINTS_BY_TD:-}" || "${MASTER_FIRST_POINTS_BY_TD^^}" == "ALL" ]]; then
    echo "ERROR: set MASTER_FIRST_POINTS_BY_TD, for example MASTER_FIRST_POINTS_BY_TD=\"120=8,210=6\"." >&2
    exit 2
fi
...
"$PY" "$FILTER_MASTER_SCRIPT" "$MASTER_PARQUET" \
    ...
    --first-points-by-td "$MASTER_FIRST_POINTS_BY_TD"
